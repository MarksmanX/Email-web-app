...
function show_email(email,mailbox) {
  ...
  body.id = "email-body";
  body.className = "col-lg-2 col-md-2 col-sm-12";
  body.innerHTML = email.body;
  emailDiv.append(body);

  const timestamp = document.createElement('div');
  timestamp.id = "email-timestamp";
  timestamp.className = "col-lg-4 col-md-4 col-sm-12";
  ...
  if(mailbox !== "sent") {
    ...
    button.className = "col-lg-1 col-md-1 col-sm-12";
...
function reply_email(email) {
  console.log('reply_email was called');
...
function mark_email_as_read(email_id, emailCard) {
  console.log('updating email as read = true');
  emailCard.style.backgroundColor = "grey";
  fetch(`/emails/${email_id}`, {
    method: 'PUT',
